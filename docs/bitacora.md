...
- **Decisiones clave y autoría:**
  - *Soporte Dual de Variable de Entorno para Bóveda (`NOTES_PATH` y `OBSIDIAN_VAULT_PATH`) (Consenso):* Se actualizaron `watcher.js`, `tools.js` y `canvas-converter.js` para leer indistintamente `NOTES_PATH` o `OBSIDIAN_VAULT_PATH`, facilitando la configuración en `.env`.
